...
function findClosestBusStop(lat, lon, busType) {
    ...
    for (let i=0;i<STOPS.length;i++) {
        let busstop = STOPS[i];
        currDistance = distance(busstop.position.latitude, busstop.position.longitude);
...
function findDistanceBetweenBusStops(origin, destination, busType) {
    ...
    let totalDistance;
    while(true) {
        if (EDGE_LIST[i%i].end == destination) {
            break;
        }
        totalDistance += EDGE_LIST[i%i].distance;
        i++;
    }
...
function estimateArrivalTime(bus, busStop, busType) {
    let closestStop = findClosestBusStop(bus.position.lat, bus.position.lon, busType);
    let distance = findDistanceBetweenBusStops(closestStop, busStop, busType);
    let estimatedTime = distance/bus.stats.avg_speed
 ...
 * @param {String} busType either 'blue' or 'red'
 */
function getNextTwoBus(vehicles, busStop, busType) {
    let timeList = [];
